...
class Operational extends StatefulWidget {
  const Operational({Key? key}) : super(key: key);

  @override
  State<Operational> createState() => _OperationalState();
}

class _OperationalState extends State<Operational> {
  ...
  loadAssetsLocal(){
    ...
    dbHelper().getDb()!.then((value) {
      value.rawQuery("SELECT * From AssetStatus WHERE assetStatus=1 AND companyId=${DataManager.getInstance().getCompanyId()}").then((value) {
        for (var i in value){
          setState(() {
