...
class _QRcodescanState extends State<QRcodescan> with SingleTickerProviderStateMixin{
  MobileScannerController cameraController = MobileScannerController(formats: [BarcodeFormat.qrCode,]);
  TextEditingController tagIdController = TextEditingController();
  List<PreventiveScheduleData> data = [];
  ...
  loadAssetsLocal(){
    ...
    dbHelper().getDb()!.then((dbValue) {
      dbValue.rawQuery("SELECT * From Schedule WHERE assetTag='${tagIdController.text}'").then((value) {
        if(value.isNotEmpty){
          for (var i in value) {
            setState(() {
          ...
          dbValue.rawQuery("SELECT * From CompletedSchedule WHERE assetTag='${tagIdController.text}'").then((value) {
            for (var i in value){
              setState(() {
                completedScheduleData.add(CompletedScheduleData(
                    ...
                    auditParamsTransaction: jsonDecode(
                        i['auditParamsTransaction'].toString()),
  ...
  loadAssetsLocal1({tagId}){
    Loading().onLoading();
    dbHelper().getDb()!.then((dbValue) {
      dbValue.rawQuery("SELECT * From Schedule WHERE assetTagId='$tagId'").then((value) {
        if(value.isNotEmpty) {
          for (var i in value) {
            setState(() {
              data.add(PreventiveScheduleData(
          ...
          dbValue.rawQuery("SELECT * From CompletedSchedule WHERE assetTagId='$tagId'").then((value) {
            for (var i in value){
              setState(() {
                completedScheduleData.add(CompletedScheduleData(
                    auditSchduleId: num.parse(i['auditSchduleId'].toString()),
            ...
            InkWell(
                onTap: (){
                  ...
                  Future.delayed(const Duration(seconds: 2),() {
                    Loading().onDone();
                    if(data.isNotEmpty && completedScheduleData.isEmpty){
                      Navigator.push(
                        context, MaterialPageRoute(builder: (context) => Asset_image(assetScheduleData: data,completedScheduleData:null),),).then((value){
                        setState(() {
                          data = [];
                        });
                    ...
                    else if(data.isEmpty && completedScheduleData.isNotEmpty){
                      Navigator.push(
                        context, MaterialPageRoute(builder: (context) => Asset_image(assetScheduleData: const [],completedScheduleData:completedScheduleData),),).then((value){
                        setState(() {
              ...
              if (barcode.rawValue == null) {
                ...
                Future.delayed(const Duration(seconds: 2), () {
                  ...
                  if(data.isNotEmpty && completedScheduleData.isEmpty){
                    ...
                    Navigator.push(
                      context, MaterialPageRoute(builder: (context) => Asset_image(assetScheduleData:[],completedScheduleData:completedScheduleData),),).then((value){
                      setState(() {
                        completedScheduleData = [];
                      });
                      tagIdController.clear();
