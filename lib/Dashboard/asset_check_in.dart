...
class AssetCheckIn extends StatefulWidget {
  PreventiveScheduleData?scheduleData;
  AssetCheckIn({Key? key, this.scheduleData}) : super(key: key);

  @override
  State<AssetCheckIn> createState() => _AssetCheckInState();
}

class _AssetCheckInState extends State<AssetCheckIn> {
  ...
  load(){
    Api().getStatusList().then((value){
      setState(() {
        ...
        selectedValue = value.rows!.first.name!;
        statusId = value.rows!.first.id!.toInt();
      });
    });
  ...
  Widget build(BuildContext context) {
    return
      ...
      Scaffold(
      appBar:Appbar().getAppbar(context, "Check In Asset"),
                ...
                children:[
                  TableRow(
                    ...
                    Padding(
                      ...
                      child: Text("${widget.scheduleData!.modelName}",style: listHeading,),
                    )
                  ]),
                  TableRow(children: [
                    const Padding(
                    ...
                    Padding(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton2(
                          isExpanded: true,
                          hint:Text(selectedValue, style: listHeading, overflow: TextOverflow.ellipsis,),
        ...
        child: ElevatedButton(
          ...
          onPressed: () {
            var data = {
              ...
              "checkin_at":dateController.text,
              "note":noteController.text
            };
            Api().assetCheckIn(assetId: widget.scheduleData!.assetTagId,bodyData: data).then((value){
              if(value['status'] == 'error'){
                ...
                dbHelper().getDb()!.then((value) {
                  value.rawUpdate(
                      ...
                      [0, 1, widget.scheduleData!.auditSchduleId]);
                });
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
          ...
          child: const Text("Check In",style: buttontextstyle),
        ),
      ),
    );
  }
