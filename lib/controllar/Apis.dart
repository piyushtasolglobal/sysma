...
class Api{
  ...
  /*Future<Preventive>getBreakdown_2() async {
  ...
  dynamic getWorkOrders({domain,cid,token}) async
  {
    dbHelper().getDb()!.then((value) {
      value.rawDelete("DELETE FROM Workorders");
      value.rawDelete("DELETE FROM WoSupporters");
  ...
  Future<Preventive>getPreventiveSchedule_2({userId,domain,cid,token}) async {
    dbHelper().getDb()!.then((value) {
      value.rawDelete("DELETE FROM Schedule");
      value.rawDelete("DELETE FROM PMHistory");
    });
